...
`packages/core/src/llm-client.ts` is the request-path source of truth. `packages/core/src/model-routing.ts` defines the operator policy contract, `packages/core/src/model-intelligence.ts` owns evidence-based ranking, and `packages/core/src/model-execution-context.ts` plus `packages/core/src/instrumented-base-agent.ts` provide concurrency-safe task attribution to normal LLM calls. Every production APEX unit uses this centralized routing client. The normal order is OpenRouter free models first, optional QwenCloud Token Plan (`qwen3.8-flash`, only when `APEX_QWEN_TOKEN_PLAN_ENABLED=true` and `QWENCLOUD_TOKEN_PLAN_API_KEY` is set), then optional direct Groq/Gemini BYOK capacity, and one reviewed paid continuity route, `z-ai/glm-5.3-flashx`, last. The Token Plan route spends the prepaid QwenCloud subscription and is not a restoration of the retired direct Qwen chain. FlashX is eligible by default when its funded OpenRouter credential is configured and bypasses APEX spend, token, request, emergency-cap, and pacing governors. `APEX_PAID_FALLBACK_ENABLED=false` is the operator's off switch: it removes the route from the chain entirely and leaves APEX free-only, parking the workforce at capacity exhaustion rather than spending. Unset or `true` keeps the reviewed default, so this changes nothing for a deployment that does not set it. Upstream OpenRouter/Z.ai billing/rate limits, error cooldowns, tool authorization, and approval policy remain authoritative. Do not add other paid providers without a new operator decision.
