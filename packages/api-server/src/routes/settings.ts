...
const BASE_INTEGRATION_CATALOG: IntegrationDefinition[] = [
  ...
  {
    id: 'qwen-byok',
    name: 'QwenCloud Token Plan — Primary BYOK',
    description: 'Operator-funded QwenCloud (qwencloud.com) Token Plan subscription — a different product from Alibaba Cloud DashScope, with its own key format (sk-sp-...) and base URL. Tried before the free OpenRouter chain on every call; governed by its own capped/paced request pool, not an unrestricted spend route.',
    category: 'ai',
    docsUrl: 'https://docs.qwencloud.com',
    envVars: [
      { key: 'APEX_QWEN_BYOK_ENABLED', label: 'Routing Enabled', placeholder: 'true or false' },
      { key: 'APEX_QWEN_REQUEST_CAP', label: 'Daily Request Cap', placeholder: '2500' },
      { key: 'APEX_QWEN_REQUEST_RATE_PER_MIN', label: 'Requests / Minute', placeholder: '25' },
      { key: 'QWEN_BASE_URL', label: 'Base URL', placeholder: 'https://token-plan.maas.qwencloudapi.com/compatible-mode/v1' },
      {
        key: 'QWEN_API_KEY',
        label: 'QwenCloud Token Plan API Key 1',
        placeholder: 'sk-sp-...',
        secret: true,
        probe: { kind: 'openai-models', baseUrl: 'https://token-plan.maas.qwencloudapi.com/compatible-mode/v1' },
      },
      {
        key: 'QWEN_API_KEY_2',
        label: 'QwenCloud Token Plan API Key 2',
        placeholder: 'sk-sp-...',
        secret: true,
        probe: { kind: 'openai-models', baseUrl: 'https://token-plan.maas.qwencloudapi.com/compatible-mode/v1' },
      },
  ...
  {
    id: 'buildmybot', name: 'BuildMyBot Connector', description: 'BuildMyBot service controls and the authenticated lead-ingest API. APEX does not open BuildMyBot\'s Supabase database.', category: 'data',
    envVars: [
      { key: 'BUILDMYBOT_APP_URL', label: 'Application URL', placeholder: 'https://www.buildmybot.app' },
      { key: 'BUILDMYBOT_API_BASE_URL', label: 'Lead ingest base URL', placeholder: 'https://www.buildmybot.app' },
      { key: 'BUILDMYBOT_LEAD_INGEST_TOKEN', label: 'Lead ingest token', placeholder: 'same value as APEX_LEAD_INGEST_TOKEN on BuildMyBot', secret: true },
