...
_Last verified against current source, production health, and CI: 2026-09-24._
...
`packages/core/src/llm-client.ts` is the request-path source of truth. `packages/core/src/model-routing.ts` defines the operator policy contract, `packages/core/src/model-intelligence.ts` owns evidence-based ranking, and `packages/core/src/model-execution-context.ts` plus `packages/core/src/instrumented-base-agent.ts` provide concurrency-safe task attribution to normal LLM calls. Every production APEX unit uses this centralized routing client. The normal order is the operator-funded QwenCloud Token Plan route (`qwen-dashscope-byok`, model `qwen3.8-flash`, via qwencloud.com — a distinct product from Alibaba Cloud Model Studio/DashScope; the provider identifier keeps its original name for continuity, see ADR-017) **first**, then OpenRouter free models, then optional direct Groq/Gemini BYOK capacity, and one reviewed paid continuity route, `z-ai/glm-5.3-flashx`, last. This is a 2026-09-23/24 operator decision (ADR-017): the operator holds a paid QwenCloud Token Plan subscription and wants it used ahead of free capacity rather than left idle. Qwen and FlashX are the only two paid routes and are architecturally distinct: FlashX is the sole **unrestricted** route — eligible by default when its funded OpenRouter credential is configured, and it bypasses APEX spend, token, request, emergency-cap, and pacing governors. Qwen is paid but **governed**: it runs on its own capped/paced request pool (`APEX_QWEN_REQUEST_CAP`, `APEX_QWEN_REQUEST_RATE_PER_MIN`) like Groq/Gemini, and every APEX workspace/emergency/per-provider governor still applies to it. `APEX_PAID_FALLBACK_ENABLED=false` is the operator's off switch for FlashX specifically: it removes that route from the chain entirely and leaves APEX free-only after Qwen, parking the workforce at capacity exhaustion rather than spending further. `APEX_QWEN_BYOK_ENABLED=false` (or leaving `QWEN_API_KEY` unset) is the equivalent off switch for Qwen, which then falls through to the free chain. Unset or `true` keeps each reviewed default, so this changes nothing for a deployment that does not set it. Upstream OpenRouter/Z.ai/QwenCloud billing/rate limits, error cooldowns, tool authorization, and approval policy remain authoritative. Do not add further paid providers beyond Qwen and FlashX without a new operator decision.
...
Nex N2.5 Mini Free is the primary model for the entire workforce unless an explicitly supported role-level free-model policy says otherwise. MiniMax M3 Free is not in the chain. Paid model IDs cannot be persisted. The paid `z-ai/glm-5.3-flashx` continuity route remains last after the free/BYOK chain and is eligible by default when its credential is configured. Free-pool, workspace-token, emergency-request, and legacy paid-spend exhaustion do not veto FlashX; only `APEX_PAID_FALLBACK_ENABLED=false` does, and it removes the route without removing any free or BYOK provider. FlashX receives the full normal conversation history instead of the smaller free-route history trim. Provider-side context/rate limits and reliability cooldowns still apply.
