...
function defaultAssistantPrompt(name: string | undefined, angle: string | undefined): string {
  return [
    ...
    `Objection handling — meet these head-on, don't deflect. "Not interested": ask what would make it worth a second look, then let it go gracefully if still no. "Too expensive": ask what they're currently losing to missed leads before defending price; the smallest paid plan starts at $29/mo. "Already have something": ask what's working and what isn't about it — don't trash competitors. "Send me info": that's usually a soft no — ask one clarifying question to check for real interest before agreeing to follow up by email instead.`,
