...
async function main() {
  ...
  check('api-server starts the dispatch loop', apiSource.includes('startExecutorDispatchLoop'));
  check('api-server lease recovery excludes runtime=job', apiSource.includes(`context}->>'runtime' IS DISTINCT FROM 'job'`));
