...
insert_after(llm, "    apiKeyEnvs: ['GEMINI_FREE_API_KEY', 'GEMINI_FREE_API_KEY_2'],\n", "    minIntervalMs: 4_000,\n")
insert_after(llm, "      'set GROQ_FREE_TIER_CONFIRMED=true only for a Groq Free plan project/key',\n", "    minIntervalMs: 2_200,\n")
insert_after(llm, "    apiKeyEnvs: ['COHERE_API_KEY'],\n", "    minIntervalMs: 3_200,\n")
insert_after(llm, "      'set POOLSIDE_FREE_ACCESS_CONFIRMED=true only while this account is on Poolside free access',\n", "    minIntervalMs: 1_500,\n")
insert_after(llm, "      'enable Alibaba Model Studio Free quota only, then set QWEN_FREE_QUOTA_ONLY=true',\n", "    minIntervalMs: 1_500,\n")
insert_after(llm, "    apiKeyEnvs: ['KILO_API_KEY'],\n", "    minIntervalMs: 1_000,\n")
insert_after(llm, "    paid: true,\n", "    minIntervalMs: 500,\n")
...
replace_once(api, "        nextResumeAt: tokenLedger.pacing.nextResumeAt,", "        nextResumeAt,")
...
replace_once(
    failure,
    r"  return /(\b429\b|\b402\b|\b404\b|\b413\b|rate limit|insufficient credits|quota|tokens per day|request too large|model is unavailable|no providers were configured|no providers .*api keys)/i.test(",
    r"  return /(\b429\b|\b402\b|\b404\b|\b413\b|\b50[234]\b|rate limit|credential in cooldown|insufficient credits|quota|tokens per day|request too large|model is unavailable|temporarily unavailable|overloaded|no providers were configured|no providers .*api keys)/i.test(",
)
...
insert_after(
    ...
    "# Provider spacing defaults can be overridden only from measured account/project limits.\n"
    "# APEX_LLM_MIN_INTERVAL_MS_GOOGLE_GEMINI=4000\n"
    "# APEX_LLM_MIN_INTERVAL_MS_GROQ=2200\n"
    "# APEX_LLM_MIN_INTERVAL_MS_COHERE=3200\n",
)

Path("scripts/verify-provider-backpressure.ts").write_text(
...
for temporary in [
    ".github/workflows/apply-backpressure-integration.yml",
    ".github/workflows/apply-backpressure-integration-v2.yml",
    ".github/workflows/apply-backpressure-integration-v3.yml",
    "scripts/apply-provider-backpressure-integration.py",
]:
    p = Path(temporary)
    if p.exists():
        p.unlink()
