...
- Do not silently restore the retired direct Gemini/Groq/Cohere/Poolside/Kilo/Mistral production provider chain outside OpenRouter. Qwen was restored on 2026-09-23 as a reviewed, governed BYOK exception under an explicit operator decision — see ADR-017. That restoration does not reopen Cohere/Poolside/Kilo/Mistral, and remains subject to the same "not silently" bar: any further change to Qwen's role in the chain needs the same review.
