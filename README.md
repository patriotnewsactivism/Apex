...
APEX tries an operator-funded, governed Qwen route first, then a free-first runtime chain, with one dedicated *unrestricted* paid continuity exception last:

1. Qwen3.8 Flash (`qwen3.8-flash`, via QwenCloud/qwencloud.com Token Plan — a distinct product from Alibaba Cloud Model Studio/DashScope) — primary, operator-funded, governed by its own capped/paced pool (ADR-017)
2. Nex N2.5 Mini Free (`nex-agi/nex-n2.5-mini:free`) — primary of the free chain
3. Nex N2.5 Pro Free (`nex-agi/nex-n2.5-pro:free`)
4. NVIDIA Nemotron 3 Super Free
5. NVIDIA Nemotron 3.5 Lightning Free
6. OpenRouter Free Router (`openrouter/free`, tool requirements preserved)
7. NVIDIA Nemotron 3 Ultra Free
8. direct Groq GPT-OSS 120B BYOK when enabled
9. direct Gemini 3.8 Flash BYOK when enabled
10. GLM 5.3 FlashX (`z-ai/glm-5.3-flashx`) — paid, unrestricted continuity, last resort

The operator-persisted OpenRouter model roster remains free-only; Qwen sits outside that roster as a separate direct BYOK route, exactly like Groq/Gemini, just tried first instead of as overflow. Set `APEX_QWEN_BYOK_ENABLED=false` (or leave `QWEN_API_KEY` unset) to remove Qwen from the chain — routing then falls through to the free chain unchanged. FlashX is a separate runtime continuity route and is eligible by default when the funded `OPENROUTER_API_KEY` is configured. Set `APEX_PAID_FALLBACK_ENABLED=false` to remove it from the chain; APEX then parks the workforce when Qwen/free/BYOK capacity are spent rather than spending further. Every free and BYOK provider is unaffected — do not unset `OPENROUTER_API_KEY` to stop paid spend, because it is also a free-pool account. Unlike FlashX, Qwen is **not** unrestricted: APEX's spend, free-request, workspace-token, provider-token, emergency-request, and pacing governors all still apply to it, on top of its own `APEX_QWEN_REQUEST_CAP`/`APEX_QWEN_REQUEST_RATE_PER_MIN` pool. APEX does not apply those same governors to FlashX specifically. Upstream OpenRouter/Z.ai/QwenCloud billing, provider limits, request timeouts, backoff/cooldowns, authentication, tool authorization, and human approvals still apply to every route.
