// ─── CI Workspace ──────────────────────────────────────────────────────────
//
// Ported from packages/cicd-automation/src/ci-workspace.ts. Original header
// comment preserved below verbatim because the rationale still applies here
// unchanged:
//
...
//   environment, distinct from what's actually deployed) and does not
//   violate Build Discipline -- that rule governs the deployed production
//   artifact, not an ephemeral, git-ignored CI sandbox used only to run
//   checks.
//
// Behavior differences from the original, both deliberate:
//   1. CI_WORKSPACE_ROOT is `<os.tmpdir()>/apex-ci-workspace` (overridable via
//      the CI_WORKSPACE_ROOT env var) instead of the hardcoded POSIX
//      `/tmp/apex-ci-workspace` -- this worker also needs to run on Windows
//      dev machines, where `/tmp` doesn't exist.
//   2. The initial-clone path uses `fs.promises.rm` instead of shelling out
//      `rm -rf` -- `rm` isn't a native command on Windows, and Node's own
//      recursive rm is portable and just as correct here.
...
const CI_WORKSPACE_ROOT = process.env.CI_WORKSPACE_ROOT ?? path.join(os.tmpdir(), 'apex-ci-workspace');
const REPO_URL = process.env.CI_REPO_URL ?? 'https://github.com/patriotnewsactivism/Apex.git';

let syncPromise: Promise<string> | null = null;

async function doSync(): Promise<string> {
  const exists = fs.existsSync(path.join(CI_WORKSPACE_ROOT, '.git'));

  if (!exists) {
    await fs.promises.rm(CI_WORKSPACE_ROOT, { recursive: true, force: true }).catch(() => {});
    await execAsync(`git clone --depth 1 ${REPO_URL} "${CI_WORKSPACE_ROOT}"`, {
      timeout: 120_000,
    });
  } else {
  ...
  const lockPath = path.join(CI_WORKSPACE_ROOT, 'pnpm-lock.yaml');
  const markerPath = path.join(CI_WORKSPACE_ROOT, '.ci-install-marker');
  ...
  const nodeModulesExists = fs.existsSync(path.join(CI_WORKSPACE_ROOT, 'node_modules'));
